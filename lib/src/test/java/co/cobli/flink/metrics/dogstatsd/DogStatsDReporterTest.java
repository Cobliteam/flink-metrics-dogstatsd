...
public class DogStatsDReporterTest {
  ...
  public void testRichTagsAndSanitization() throws Exception {
    ...
    c.inc(5);
    reporter.report(); // Baseline (First run sends nothing for counters in Delta mode)

    c.inc(10);
    reporter.report(); // Delta: 10
    ...
    assertTrue(
      "Expected Delta value '10|c'. Packet: " + packet,
      packet.contains(":10|c")
    );
  ...
  public void testCounterLifecycle() throws Exception {
    ...
    // 1. First report sets baseline, no packet expected for Delta logic
    counter.inc(10);
    reporter.report();
    drainSocket(serverSocket);

    // 2. Increment 5 -> delta is 5 -> report 5|c
    counter.inc(5);
    reporter.report();
    assertPacketContains("test.counter:5|c");

    // 3. Zero heartbeat test
    reporter.report(); // No increment
    assertPacketContains("test.counter:0|c"); // Explicit 0 sent
  }
  ...
  public void testHistogramDeltaAndStats() throws Exception {
    ...
    reporter.report(); // Baseline set

    // Histogram Stats (Gauge) are always sent
    List<String> lines = receiveAllLines(serverSocket, 1);
  ...
  public void testHistogramsAndMeters() throws Exception {
    ...
    assertTrue(
      lines.stream().anyMatch(l -> l.contains("my.meter.count:100|g"))
    );
  }

  @Test
  public void testResilienceToPacketLoss() throws Exception {
    SimpleCounter c = new SimpleCounter();
    reporter.notifyOfAddedMetric(c, "resilient.counter", mockGroup);
    ...
    // 2. Simulate packet loss (write failure)
    c.inc(5); // Delta 5
    reporter.shouldFailWrite = true;
    reporter.report();
    assertNoPacket("Simulated packet loss should yield no data");
    ...
    // With Deltas, we only see the new delta (1), the 5 was lost.
    assertPacketContains("resilient.counter:1|c");
  }
