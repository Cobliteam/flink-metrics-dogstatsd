  ...
  implements MetricReporter, Scheduled, CharacterFilter
{
  ...
  public void close() {
    ...
    // Clear state maps to avoid leaks on restart
  ...
  private void reportCounter(
    ...
    previousCounterValues.compute(counter, (k, previous) -> {
      ...
      if (previous == null) return current; // First run: set baseline, don't send to avoid spikes.
  ...
  private void reportMeter(DatagramChannel ch, DMetric dMetric, Meter meter) {
    ...
    // Renamed to .count to be standard
    bufferMetric(
      ch,
      dMetric.cleanName + ".count",
  ...
  private void cleanupState() {
    // Sync state maps with registry maps to prevent leaks if notification is missed
